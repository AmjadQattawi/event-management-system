...
📌 Overview
The Event Management System is a fully-featured backend API that handles 
the complete lifecycle of events — from creation and publishing to booking, 
payment processing, and post-event reviews. The system integrates real-time 
weather data for event locations, sends automated email notifications after 
successful payments, and uses scheduled jobs to manage booking expirations 
automatically.

Built as a graduation-to-industry project during a professional Java/Spring 
Boot internship, it demonstrates real-world backend engineering practices 
including clean layered architecture, JWT-secured endpoints, unit testing, 
structured logging, database migrations, async processing, and external API 
integration.                                                      // NEW line

✨ Key Features
Feature                  | Description
-------------------------|----------------------------------------------------
🗂️ Event Lifecycle       | Full CRUD with status workflow: DRAFT → PUBLISHED → CANCELLED
...
🔐 Security              | JWT-based authentication with Spring Security — all endpoints protected  // NEW
🧪 Unit Testing          | Core business logic covered with JUnit 5 & Mockito                      // NEW
📋 Logging               | Structured runtime logging via SLF4J for traceability and debugging     // NEW
...
├── configuration    → App config (async, scheduling, security, beans)  // UPDATED
├── security         → JWT filter, token provider, security config       // NEW
...
- Security Filter Chain via Spring Security for stateless JWT validation  // NEW
...
Security           | Spring Security + JWT (jjwt)               // NEW
...
JWT_SECRET=your_jwt_secret_key_here                   // NEW
...
Note: Liquibase will automatically create all tables and sequences on 
first run. No manual SQL execution required.
...
Auth        | /auth       | Register, Login → returns JWT token             // NEW
...
Booking Flow
- Attendee requests a booking → system validates event is PUBLISHED and has available capacity
- Booking created with status PENDING and capacity is reserved
- Attendee must complete payment within 15 minutes or booking is auto-cancelled
- On successful payment → booking moves to CONFIRMED and attendee earns reward points
...
Weather Integration
- On event creation, the system geocodes the event location via Open-Meteo's Geocoding API
...
Filters are combined dynamically using JPA Specifications — only the 
...
│   │   ├── repository/
│   │   ├── searchCriteria/
│   │   ├── security/          ← JWT filter, token provider, config  // NEW
│   │   ├── service/
│   │   ├── specification/
│   │   └── validator/
│   └── resources/
...
└── test/
    └── java/com/eventmanagement/   ← JUnit & Mockito test classes   // NEW
